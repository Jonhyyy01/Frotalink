...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['confirm']) && $_POST['confirm'] === 'yes') {
        $stmt = $conn->prepare('DELETE FROM cargas WHERE id = ?');
...
$stmt = $conn->prepare('SELECT codigo_rastreio FROM cargas WHERE id = ? LIMIT 1');
$stmt->bind_param('i', $id);
$stmt->execute();
$stmt->bind_result($codigo_rastreio);
if (! $stmt->fetch()) {
    $stmt->close();
    header('Location: cargas_listar.php');
    exit;
...
<head>
    ...
    <title>Apagar Carga - Frotalink</title>
    ...
    <header class="topbar">
        <div class="brand">
            ...
            <div>
                ...
                <span class="brand-subtitle">Apagar Carga</span>
            </div>
        </div>
    ...
    <div class="page-layout">
        ...
        <main class="dashboard-content">
            <section class="widget">
                ...
                <p>Tem a certeza que pretende apagar a carga <strong><?php echo htmlspecialchars($codigo_rastreio); ?></strong>?</p>

                <form method="post" action="cargas_apagar.php?id=<?php echo urlencode($id); ?>">
                    <button type="submit" name="confirm" value="yes" class="button danger">Sim, apagar</button>
                    <a href="cargas_listar.php" class="button secondary">Cancelar</a>
                </form>
            </section>
        </main>
    </div>
