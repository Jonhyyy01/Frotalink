...
$stmt = $conn->prepare('SELECT nome_completo, data_nascimento, telefone, email, nif, numero_carta_conducao, validade_carta, categoria_carta, numero_cam_cqm, validade_cam, numero_mecanografico, data_admissao, tipo_contrato, estado, disponibilidade, viatura_atual_id, user_id FROM motoristas WHERE id = ? LIMIT 1');
$stmt->bind_param('i', $id);
$stmt->execute();
$stmt->bind_result($nome_completo, $data_nascimento, $telefone, $email, $nif, $numero_carta_conducao, $validade_carta, $categoria_carta, $numero_cam_cqm, $validade_cam, $numero_mecanografico, $data_admissao, $tipo_contrato, $estado, $disponibilidade, $viatura_atual_id, $user_id);
if (! $stmt->fetch()) {
    $stmt->close();
    header('Location: motoristas_listar.php');
    exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome_completo === '') {
        ...
        $stmt = $conn->prepare('UPDATE motoristas SET nome_completo = ?, data_nascimento = ?, telefone = ?, email = ?, nif = ?, numero_carta_conducao = ?, validade_carta = ?, categoria_carta = ?, numero_cam_cqm = ?, validade_cam = ?, numero_mecanografico = ?, data_admissao = ?, tipo_contrato = ?, estado = ?, disponibilidade = ?, viatura_atual_id = ?, user_id = ? WHERE id = ?');
        if ($stmt) {
            $stmt->bind_param('sssssssssssssssiii', $nome_completo, $data_nascimento, $telefone, $email, $nif, $numero_carta_conducao, $validade_carta, $categoria_carta, $numero_cam_cqm, $validade_cam, $numero_mecanografico, $data_admissao, $tipo_contrato, $estado, $disponibilidade, $viatura_atual_id, $user_id, $id);
            if ($stmt->execute()) {
                header('Location: motoristas_listar.php');
                exit;
            }
...
<head>
    ...
    <title>Editar Motorista - Frotalink</title>
    ...
    <header class="topbar">
        <div class="brand">
            ...
            <div>
                ...
                <span class="brand-subtitle">Editar Motorista</span>
            </div>
        </div>
    ...
    <div class="page-layout">
        ...
        <main class="dashboard-content">
            <section class="widget">
                ...
                <form method="post" action="motoristas_editar.php?id=<?php echo urlencode($id); ?>">
