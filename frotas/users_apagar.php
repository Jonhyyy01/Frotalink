...
$stmt = $conn->prepare('SELECT nome, email, nivel_acesso FROM utilizadores WHERE id = ? LIMIT 1');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($email === 'admin@frota.local' || $nivel_acesso === 'admin') {
        header('Location: users_listar.php');
        exit;
    }
...
<head>
    ...
    <title>Apagar Utilizador - Frotalink</title>
    ...
    <header class="topbar">
        <div class="brand">
            ...
            <div>
                ...
                <span class="brand-subtitle">Apagar Utilizador</span>
            </div>
        </div>
    ...
    <div class="page-layout">
        ...
        <main class="dashboard-content">
            <section class="widget">
                ...
                <p>Tem certeza que deseja apagar o utilizador:</p>
                <p><strong><?php echo htmlspecialchars($nome); ?></strong> (<em><?php echo htmlspecialchars($email); ?></em>)</p>
                <form method="post" action="users_apagar.php?id=<?php echo urlencode($id); ?>">
                    <button type="submit" class="button danger">Sim, apagar</button>
                    <a class="button secondary" href="users_listar.php">Cancelar</a>
                </form>
            </section>
        </main>
    </div>
