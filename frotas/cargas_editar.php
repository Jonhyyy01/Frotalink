...
$stmt = $conn->prepare('SELECT codigo_rastreio, estado_carga, descricao, tipo_carga, peso_kg, volume_m3, quantidade_paletes, local_recolha, data_hora_recolha_prevista, data_hora_recolha_real, local_entrega, data_hora_entrega_prevista, data_hora_entrega_real, cliente_id, valor_transporte, pago, viatura_id, motorista_id FROM cargas WHERE id = ? LIMIT 1');
$stmt->bind_param('i', $id);
$stmt->execute();
$stmt->bind_result($codigo_rastreio, $estado_carga, $descricao, $tipo_carga, $peso_kg, $volume_m3, $quantidade_paletes, $local_recolha, $data_hora_recolha_prevista, $data_hora_recolha_real, $local_entrega, $data_hora_entrega_prevista, $data_hora_entrega_real, $cliente_id, $valor_transporte, $pago, $viatura_id, $motorista_id);
if (! $stmt->fetch()) {
    $stmt->close();
    header('Location: cargas_listar.php');
    exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($codigo_rastreio === '') {
        ...
        $stmt = $conn->prepare('UPDATE cargas SET codigo_rastreio = ?, estado_carga = ?, descricao = ?, tipo_carga = ?, peso_kg = ?, volume_m3 = ?, quantidade_paletes = ?, local_recolha = ?, data_hora_recolha_prevista = ?, data_hora_recolha_real = ?, local_entrega = ?, data_hora_entrega_prevista = ?, data_hora_entrega_real = ?, cliente_id = ?, valor_transporte = ?, pago = ?, viatura_id = ?, motorista_id = ? WHERE id = ?');
        if ($stmt) {
            $stmt->bind_param('ssssddissssssidiiii', $codigo_rastreio, $estado_carga, $descricao, $tipo_carga, $peso_kg, $volume_m3, $quantidade_paletes, $local_recolha, $data_hora_recolha_prevista, $data_hora_recolha_real, $local_entrega, $data_hora_entrega_prevista, $data_hora_entrega_real, $cliente_id, $valor_transporte, $pago, $viatura_id, $motorista_id, $id);
            if ($stmt->execute()) {
                header('Location: cargas_listar.php');
                exit;
            }
...
<head>
    ...
    <title>Editar Carga - Frotalink</title>
    ...
    <header class="topbar">
        <div class="brand">
            ...
            <div>
                ...
                <span class="brand-subtitle">Editar Carga</span>
            </div>
        </div>
    ...
    <div class="page-layout">
        ...
        <main class="dashboard-content">
            <section class="widget">
                ...
                <form method="post" action="cargas_editar.php?id=<?php echo urlencode($id); ?>">
