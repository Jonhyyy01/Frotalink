...
$stmt = $conn->prepare('SELECT tipo_cliente, nome, responsavel_contacto, nif_nipc, morada_fiscal, codigo_postal, localidade, pais, telefone, email, website, limite_credito, prazo_pagamento_dias, estado_cliente, lat, lon FROM clientes WHERE id = ? LIMIT 1');
$stmt->bind_param('i', $id);
$stmt->execute();
$stmt->bind_result($tipo_cliente, $nome, $responsavel_contacto, $nif_nipc, $morada_fiscal, $codigo_postal, $localidade, $pais, $telefone, $email, $website, $limite_credito, $prazo_pagamento_dias, $estado_cliente, $lat, $lon);
if (! $stmt->fetch()) {
    $stmt->close();
    header('Location: clientes_listar.php');
    exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome === '' || $nif_nipc === '') {
        ...
        $stmt = $conn->prepare('UPDATE clientes SET tipo_cliente = ?, nome = ?, responsavel_contacto = ?, nif_nipc = ?, morada_fiscal = ?, codigo_postal = ?, localidade = ?, pais = ?, telefone = ?, email = ?, website = ?, limite_credito = ?, prazo_pagamento_dias = ?, estado_cliente = ?, lat = ?, lon = ? WHERE id = ?');
        if ($stmt) {
            $stmt->bind_param('sssssssssssdisddi', $tipo_cliente, $nome, $responsavel_contacto, $nif_nipc, $morada_fiscal, $codigo_postal, $localidade, $pais, $telefone, $email, $website, $limite_credito, $prazo_pagamento_dias, $estado_cliente, $lat, $lon, $id);
            if ($stmt->execute()) {
                header('Location: clientes_listar.php');
                exit;
            }
            $error = 'Erro ao atualizar cliente. Verifique se o NIF/NIPC é único.';
            $stmt->close();
        } else {
...
<head>
    ...
    <title>Editar Cliente - Frotalink</title>
    ...
    <header class="topbar">
        <div class="brand">
            ...
            <div>
                ...
                <span class="brand-subtitle">Editar Cliente</span>
            </div>
        </div>
    ...
    <div class="page-layout">
        ...
        <main class="dashboard-content">
            <section class="widget">
                ...
                <form method="post" action="clientes_editar.php?id=<?php echo urlencode($id); ?>">
